...
5. **長コードは file 必須**: チャットに数百行を貼らず `<file>` で保存し、作成後 `python -m py_compile` / `npm run build` 等で検証する。
...
- ファイル操作タグ（<file> <edit> <replace> <read_file> <list_dir>）は、その耐久ワークスペースがルートです。path はワークスペースからの相対パスで指定してください（例: myproject/main.py）。親フォルダは自動作成されます。絶対パスや /workspace/ プレフィックスも自動で相対パスに正規化されますが、ワークスペース外への保存はできません（散乱防止のための仕様です）。
