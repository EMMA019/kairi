...
async def chat(request: ChatRequest):
    ...
    from app.core.demo_mode import demo_chat_sse, demo_enabled

    if demo_enabled():
        return StreamingResponse(
            demo_chat_sse(user_input),
        ...
        while supervisor_loop_count < max_supervisor_loops:
            ...
            if is_weekend_news_query(user_input):
                instruction = (
                    ...
                    "引用したソースに書いてある事実だけ書け。同じURLから別試合・別選手を足すな。"
                    "「一方」で横道にそれるな。ソースに無い固有名を出すな。"
                    "予報・展望・最終調整・配信案内・試合情報ページ・星空・2週間天気は出すな。"
                ...
                instruction = (
                    "【ニュース本文】無料で読める報道だけ書け。"
                    "引用したソースに書いてある事実だけ書け。同じURLから別試合を足すな。"
                    "予報・配信案内・試合情報ページは出すな。"
