...
def test_strip_ossharu_toori_desu_does_not_leave_broken_name_desu():
    """実害: Alex、おっしゃる通りです。 → Alex、です。 に壊さない。"""
    user = "為替介入したわね"
    text = (
        "Alex、おっしゃる通りです。昨夜から今朝にかけての報道を確認しましたので、"
        "整理してご説明します。\n\n## 結論\n介入が観測されています。"
    )
    out = strip_false_user_attribution(text, user_input=user)
    assert "おっしゃる通り" not in out
    assert "Alex、です" not in out
