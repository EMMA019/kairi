...
// ── Investment Highlights（無料・ルールベース） ──────────────────────────────
function InvestmentHighlights({ ticker, data, quote, allData }) {
  if (!data) return null;

  const rs        = data.scores?.rs || 0;
  const ecr       = data.scores?.ecr_rank || 0;
  const cmp       = data.scores?.composite || 0;
  const ses       = data.scores?.ses || 0;
  const vcp       = data.scores?.vcp || 0;
  const eps       = data.canslim_metrics?.eps_growth;
  const rev       = data.canslim_metrics?.rev_growth;
  const ma50      = data.ma50_ratio || 0;
  const ma200     = data.ma200_ratio || 0;
  const pivotDist = data.pivot_dist_pct || 0;
  const grade     = data.canslim_grade || '-';
  ...
  if (rs >= 90)       positives.push(`RS${rs}：市場上位${100-rs}%のアウトパフォーマー`);
  else if (rs >= 80)  positives.push(`RS${rs}：相場全体に対して強い相対力を維持`);
  else                negatives.push(`RS${rs}：相対力がやや低く、資金流入が限定的`);

  if (ecr >= 70)      positives.push(`ECR${ecr}：強い資金流入トレンドを確認`);
  else if (ecr >= 60) positives.push(`ECR${ecr}：資金流入が継続中`);
  else                negatives.push(`ECR${ecr}：資金流入の勢いが弱い`);

  if (eps != null) {
    if (eps >= 50)    positives.push(`EPS成長+${eps.toFixed(1)}%：高い収益成長力`);
    else if (eps >= 20) positives.push(`EPS成長+${eps.toFixed(1)}%：安定した収益成長`);
    else if (eps < 0) negatives.push(`EPS成長${eps.toFixed(1)}%：収益が減少傾向`);
  }

  if (rev != null) {
    if (rev >= 20)    positives.push(`売上成長+${rev.toFixed(1)}%：トップライン拡大が加速`);
    else if (rev >= 10) positives.push(`売上成長+${rev.toFixed(1)}%：安定した売上拡大`);
    else if (rev < 0) negatives.push(`売上成長${rev.toFixed(1)}%：売上が伸び悩み`);
  }

  if (ses >= 80)      positives.push(`SES${ses}：資金流入の勢いが非常に強い`);
  else if (ses < 30)  negatives.push(`SES${ses}：資金の勢いが弱く、モメンタムに欠ける`);

  if (ma50 > 20)      negatives.push(`MA50から+${ma50.toFixed(1)}%：短期的に過熱気味`);
  else if (ma50 > 0)  positives.push(`MA50から+${ma50.toFixed(1)}%：健全なトレンド上昇中`);
  else                negatives.push(`MA50を下回り：短期トレンドが崩れている`);

  if (ma200 > 50)     negatives.push(`MA200から+${ma200.toFixed(1)}%：長期的に過熱気味、押し目注意`);
  else if (ma200 > 0) positives.push(`MA200から+${ma200.toFixed(1)}%：長期上昇トレンド継続中`);

  if (pivotDist > 5)        negatives.push(`ピボットから+${pivotDist.toFixed(1)}%：ピボット上抜けすぎ、エントリーは押し目待ち`);
  else if (pivotDist >= -3) positives.push(`ピボット付近（${pivotDist.toFixed(1)}%）：理想的なエントリーゾーン`);
  else                      positives.push(`ピボット手前（${pivotDist.toFixed(1)}%）：ブレイクアウト準備中`);

  if (grade === 'A+' || grade === 'A') positives.push(`ファンダグレード${grade}：財務・成長性ともに優秀`);
  else if (grade === 'B' || grade === 'B+') negatives.push(`ファンダグレード${grade}：ファンダはやや平均的`);

  // ── Better Bets（同セクターで上位銘柄） ──
  const betterBets = [];
  if (allData && sector) {
    const sectorPeers = allData
      .filter(d => d.sector === sector && d.ticker !== ticker)
      .map(d => ({
        ticker: d.ticker,
        composite: d.scores?.composite || 0,
        rs: d.scores?.rs || 0,
        ecr: d.scores?.ecr_rank || 0,
        grade: d.canslim_grade || '-',
      }))
      .filter(d => d.composite > cmp && d.rs > rs)
      .sort((a, b) => b.composite - a.composite)
      .slice(0, 3);
    betterBets.push(...sectorPeers);
  }

  // ── サマリー ──
  const bullish = positives.length > negatives.length;
  const summary = bullish
    ? `全体的にポジティブ。RS・ECRともに高水準で資金流入が継続中。${pivotDist >= -3 && pivotDist <= 5 ? 'ピボット付近でエントリーを検討できる水準。' : ''}`
    : `ポジティブな要素もあるが懸念点も存在。スコアの改善を確認してからエントリーを検討するのが安全。`;

  return (
    <div style={{ background:'#0d0d0d', border:'1px solid #222', borderRadius:10, padding:20 }}>
      <div style={{ fontFamily:'monospace', fontSize:11, color:'#888', marginBottom:12, display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ color:'#ffb800' }}>●</span> Investment Highlights
        <span style={{ marginLeft:'auto', fontSize:10, color:'#444' }}>strategies.jsonベース・無料</span>
      </div>

      <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:12 }}>
        {positives.map((h, i) => (
          <div key={i} style={{ display:'flex', gap:8, alignItems:'flex-start', fontFamily:'monospace', fontSize:11 }}>
        ...
        {negatives.map((h, i) => (
          <div key={i} style={{ display:'flex', gap:8, alignItems:'flex-start', fontFamily:'monospace', fontSize:11 }}>
            <span style={{ color:'#ff3355', flexShrink:0 }}>▼</span>
            <span style={{ color:'#ccc' }}>{h}</span>
          </div>
        ))}
      </div>

      <div style={{ borderTop:'1px solid #1e1e1e', paddingTop:10, fontFamily:'monospace', fontSize:10, color:'#666', lineHeight:1.6, marginBottom: betterBets.length > 0 ? 12 : 0 }}>
        {summary}
      </div>

      {betterBets.length > 0 && (
        <div style={{ borderTop:'1px solid #1e1e1e', paddingTop:10 }}>
          <div style={{ fontFamily:'monospace', fontSize:10, color:'#00aaff', marginBottom:8 }}>
            // BETTER BETS — 同セクターで上位の銘柄
          </div>
          {betterBets.map((b, i) => (
            <div key={i} style={{ display:'flex', gap:10, alignItems:'center', fontFamily:'monospace', fontSize:11, marginBottom:4 }}>
              <span style={{ color:'#00aaff', fontWeight:'bold', minWidth:52 }}>{b.ticker}</span>
              <span style={{ color:'#666' }}>CMP {b.composite} / RS {b.rs} / ECR {b.ecr}</span>
              <span style={{ color:'#888', marginLeft:'auto' }}>Grade {b.grade}</span>
            </div>
          ))}
        </div>
      )}
...
// ── Peer Comparison（無料・strategies.jsonから同セクター抽出） ────────────────
function PeerComparison({ ticker, data, allData }) {
  ...
  const ecr    = data.scores?.ecr_rank || 0;
  const cmp    = data.scores?.composite || 0;
  const eps    = data.canslim_metrics?.eps_growth || 0;

  const peers = allData
    .filter(d => d.sector === sector && d.ticker !== ticker && (d.scores?.rs || 0) >= 70)
    .map(d => {
      const pRs  = d.scores?.rs || 0;
      const pEcr = d.scores?.ecr_rank || 0;
      const pCmp = d.scores?.composite || 0;
      const pEps = d.canslim_metrics?.eps_growth || 0;
      const wins = [pRs > rs, pEcr > ecr, pCmp > cmp, pEps > eps].filter(Boolean).length;
      const verdict = wins >= 3 ? 'better' : wins <= 1 ? 'worse' : 'similar';
      const vs = [];
      if (pRs > rs + 3)   vs.push(`RS${pRs}(↑)`);
      else if (pRs < rs - 3) vs.push(`RS${pRs}(↓)`);
      if (pCmp > cmp + 5) vs.push(`CMP${pCmp.toFixed(0)}(↑)`);
      else if (pCmp < cmp - 5) vs.push(`CMP${pCmp.toFixed(0)}(↓)`);
      if (pEps > eps + 10) vs.push(`EPS+${pEps.toFixed(0)}%(↑)`);
      return { ticker: d.ticker, vs: vs.join(' / ') || '同水準', verdict, composite: pCmp };
    })
    .sort((a, b) => b.composite - a.composite)
    .slice(0, 5);
  ...
  const verdictColor = v => v === 'better' ? '#00ff88' : v === 'worse' ? '#ff3355' : '#ffb800';
  const verdictLabel = v => v === 'better' ? '↑優位' : v === 'worse' ? '↓劣位' : '→同程度';

  return (
    <div style={{ background:'#0d0d0d', border:'1px solid #222', borderRadius:10, padding:20 }}>
      <div style={{ fontFamily:'monospace', fontSize:11, color:'#888', marginBottom:12, display:'flex', alignItems:'center', gap:8 }}>
        <span style={{ color:'#aa66ff' }}>●</span> Peer Comparison
        <span style={{ marginLeft:'auto', fontSize:10, color:'#444' }}>{sector || '同セクター'}</span>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        {peers.map((p, i) => (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:10, fontFamily:'monospace', fontSize:11, borderBottom:'1px solid #1a1a1a', paddingBottom:6 }}>
            <Link to={`/realtime/${p.ticker}`} style={{ color:'#4499ff', fontWeight:'bold', minWidth:52, textDecoration:'none', fontFamily:'monospace', fontSize:11 }}>
              {p.ticker} ↗
            </Link>
            <span style={{ color:'#555', flex:1 }}>{p.vs}</span>
            <span style={{ fontWeight:'bold', color: verdictColor(p.verdict), minWidth:52, textAlign:'right' }}>
  ...
  return (
    <div className="min-h-screen bg-ink pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <InvestmentHighlights ticker={ticker} data={data} quote={quote} allData={allData} />

        {/* ── Peer Comparison ── */}
        <PeerComparison ticker={ticker} data={data} allData={allData} />
