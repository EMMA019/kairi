...
2. **時制・日付・四半期情報の反映**: 対象の日付がある場合は、クエリに「月日のみ」を含め、架空の年号（例:2026年）は絶対にクエリに含めないでください。実際の検索エンジンは現実の年でインデックスされているため、"2026"をクエリに含めると最新のニュースがヒットしなくなります。システム側で自動的に直近の期間フィルタをかけるため、クエリはシンプルにしてください。
...
5. **英語クエリの原則**: 検索エンジンは英語のほうが圧倒的にヒット率・情報の質が高いため、日本国内のローカルな話題以外はすべて英語で同義クエリを1〜2個（最大2個）生成してください。ニュース・政治経済のクエリには "2026" の年号を絶対に含めないこと。例えば「世界の政治経済ニュース」→ `["world politics economy news current", "global economic news July"]` のように、年号を省き月や"current"等の単語を使用してください。
...
6.5. **施設料金・営業時間・チケット価格の「公式サイト検索」ルール (🟡 P1)**: 観光地・水族館・施設等の「料金」「営業時間」「定休日」に関する質問時は、まっぷるやじゃらん等のまとめ記事で止まらず正確な最新料金（例：改定後料金）を確認できるように、必ず「施設名 料金 営業時間 公式」「施設名 入園料 最新 公式サイト」のような公式サイト・公式案内をヒットさせるキーワードを生成すること。
...
6.7. **企業のCEO・経営陣・組織トップに関する検索クエリ (🔴 P0)**: 企業や団体のCEO、役員体制、人事異動に関する質問時は、必ず最新情報を取得できるキーワード（例: `["Company name current CEO leadership", "企業名 現CEO 経営陣 最新"]`）を生成してください。"2026"という年号は検索エンジンのノイズになるため含めないでください。
...
【判定ルール（優先順位順）】
1. 【最優先】「なんか熱い銘柄ない？」「今日熱い銘柄は？」など具体的なティッカー・セクター名が含まれない単純な「注目銘柄」の質問は `needs_search: false` としますが、ユーザーが「半導体」「リバランス」「ポートフォリオ」「決算」「インフレ」等のセクターや市場見通しに言及しながら組み入れ候補・今後の動向を尋ねている場合（例：「半導体は短期狙いだった。組み込むなら？」「半導体比率が高いのでリバランスしたい、何がいい？」等）は、必ず `needs_search: true` とし、「半導体株 ETF リバウンド 見通し 最新」「半導体セクター 注目銘柄 リバランス current」「semiconductor ETF stock outlook current」のような検索用キーワードクエリ配列を出力してください。
